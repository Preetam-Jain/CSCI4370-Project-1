...
public class Relation implements uga.cs4370.mydb.Relation {
    ...
    */
    ...
    public void insert(Cell... cells) {
        ...
        Integer nullVar = null; // cant compare ints and doubles to null so made a variable that can store and compare it
        Double nullV = null; // 
        for (Cell cell : cells) {
            if (types.get(index) == Type.INTEGER && cell.getAsInt()!=nullVar) {
                row.add(cell);
            } 
            else if (types.get(index) == Type.DOUBLE && cell.getAsDouble()!=nullV) {
                row.add(cell);
            } 
            else if (types.get(index) == Type.STRING && !cell.getAsString().equals(null)) {
                row.add(cell);
            } 
            else {
                throw new IllegalArgumentException("Cell does not correspond to attribute type");
            }
            index++;
        }
        this.table.add(row);
